...
import numpy as np
import h5py
from HiggsAnalysis.CombinedLimit.new_tfh5pyutils import maketensor,makesparsetensor
from HiggsAnalysis.CombinedLimit.new_tfsparseutils import simple_sparse_tensor_dense_matmul, simple_sparse_slice0begin, simple_sparse_to_dense, SimpleSparseTensor, makeCache
from HiggsAnalysis.CombinedLimit.new_lsr1trustobs import SR1TrustExact
import scipy
import math
import time
import hashlib
...
# import sys
# sys.path.append("/eos/home-s/skamble/lfu/combineTF/CMSSW_14_1_0_pre4/lib/el9_amd64_gcc12")
import ROOT
...
from HiggsAnalysis.CombinedLimit.new_tfscipyhess import ScipyTROptimizerInterface,jacobian,sum_loop
...
# def fill_hist_from_array(array, hist):
#     """Fill a ROOT TH2 histogram from a numpy array."""
...
#     """
#     Fill a ROOT histogram from a numpy array, optionally including errors.

#     Parameters:
#     - array: The numpy array containing the histogram content.
#     - hist: The ROOT histogram to fill.
#     - errors: An optional numpy array with the same shape as `array` containing the errors.
#     """
...
def array2hist(array, hist, errors=None):
    ...
    if len(array.shape) == 1:
        ...
        raise ValueError("Unsupported array dimension. Only 1D and 2D arrays are supported.")
...
print(f"hprocs (Process Histograms) has shape:", procs.shape)
print(f"hsignals (Signal Histograms) has shape:", signals.shape)
print(f"hsysts (Systematic Uncertainty Histograms) has shape:", systs.shape)
print(f"hsystsnoprofile (Systematic Uncertainty Histograms (No Profile)) has shape:", systsnoprofile.shape)
print(f"hsystsnoconstraint (Systematic Uncertainty Histograms (No Constraint)) has shape:", systsnoconstraint.shape)
print(f"hsystgroups (Systematic Uncertainty Groups) has shape:", systgroups.shape)
print(f"hsystgroupidxs (Systematic Uncertainty Group Indexes) has shape:", systgroupidxs.shape)
print(f"hchargegroups (Charge Groups) has shape:", chargegroups.shape)
print(f"hchargegroupidxs (Charge Group Indexes) has shape:", chargegroupidxs.shape)
print(f"hpolgroups (Polarization Groups) has shape:", polgroups.shape)
print(f"hpolgroupidxs (Polarization Group Indexes) has shape:", polgroupidxs.shape)
print(f"hhelgroups (Helicity Groups) has shape:", helgroups.shape)
print(f"hhelgroupidxs (Helicity Group Indexes) has shape:", helgroupidxs.shape)
print(f"hsumgroups (Sum Groups) has shape:", sumgroups.shape)
print(f"hsumgroupsegmentids (Sum Group Segment IDs) has shape:", sumgroupsegmentids.shape)
print(f"hsumgroupidxs (Sum Group Indexes) has shape:", sumgroupidxs.shape)
print(f"hchargemetagroups (Charge Metagroups) has shape:", chargemetagroups.shape)
print(f"hchargemetagroupidxs (Charge Metagroup Indexes) has shape:", chargemetagroupidxs.shape)
print(f"hratiometagroups (Ratio Metagroups) has shape:", ratiometagroups.shape)
print(f"hratiometagroupidxs (Ratio Metagroup Indexes) has shape:", ratiometagroupidxs.shape)
print(f"hhelmetagroups (Helicity Metagroups) has shape:", helmetagroups.shape)
print(f"hhelmetagroupidxs (Helicity Metagroup Indexes) has shape:", helmetagroupidxs.shape)
print(f"hreggroups (Regression Groups) has shape:", reggroups.shape)
print(f"hreggroupidxs (Regression Group Indexes) has shape:", reggroupidxs.shape)
print(f"hpoly1dreggroups (Polynomial 1D Regression Groups) has shape:", poly1dreggroups.shape)
print(f"hpoly1dreggroupfirstorder (Polynomial 1D Regression Groups (First Order)) has shape:", poly1dreggroupfirstorder.shape)
print(f"hpoly1dreggrouplastorder (Polynomial 1D Regression Groups (Last Order)) has shape:", poly1dreggrouplastorder.shape)
print(f"hpoly1dreggroupnames (Polynomial 1D Regression Group Names) has shape:", poly1dreggroupnames.shape)
print(f"hpoly1dreggroupbincenters (Polynomial 1D Regression Group Bin Centers) has shape:", poly1dreggroupbincenters.shape)
print(f"hpoly2dreggroups (Polynomial 2D Regression Groups) has shape:", poly2dreggroups.shape)
print(f"hpoly2dreggroupfirstorder (Polynomial 2D Regression Groups (First Order)) has shape:", poly2dreggroupfirstorder.shape)
print(f"hpoly2dreggrouplastorder (Polynomial 2D Regression Groups (Last Order)) has shape:", poly2dreggrouplastorder.shape)
print(f"hpoly2dreggroupfullorder (Polynomial 2D Regression Groups (Full Order)) has shape:", poly2dreggroupfullorder.shape)
print(f"hpoly2dreggroupnames (Polynomial 2D Regression Group Names) has shape:", poly2dreggroupnames.shape)
print(f"hpoly2dreggroupbincenters0 (Polynomial 2D Regression Group Bin Centers (0)) has shape:", poly2dreggroupbincenters0.shape)
print(f"hpoly2dreggroupbincenters1 (Polynomial 2D Regression Group Bin Centers (1)) has shape:", poly2dreggroupbincenters1.shape)
print(f"hnoigroups (NOI Groups) has shape:", noigroups.shape)
print(f"hnoigroupidxs (NOI Group Indexes) has shape:", noigroupidxs.shape)
print(f"hmaskedchans (Masked Channels) has shape:", maskedchans.shape)
print("\n")
...
print("\nnbins = %d, nbinsfull = %d, nproc = %d, npoi = %d, nsyst = %d, nsystnoprofile = %d" % (nbins,nbinsfull,nproc, npoi, nsyst, nsystnoprofile))
print("\n")
...
def is_blind(syst):
  return syst.startswith(b"massShiftW")
...
if do_blinding and not (options.unblind_fit_result and options.unblind_value):
  ...
  offset_seed = int(hashlib.sha256(seed_string.encode()).hexdigest()[:8], 16)
...
if options.POIMode == "mu":
  for signal in signals:
      # Decode signal if it's a bytes object
      if isinstance(signal, bytes):
          signal = signal.decode('utf-8')
      outputname.append(f"{signal}_{options.POIMode}")
...
# tree.Branch('satnllvalfull',tsatnllvalfull,'satnllvalfull/D')   # temporarily removing as got error in doNuisanceReport.py
...
for syst in systs:
  systname = syst
  # Ensure the systname is a string
  if isinstance(systname, bytes):
    systname = systname.decode('utf-8')
  tthetaval = array('f', [0.])
...
print("\nInitializing variables (this will trigger loading of large arrays from disk)\n")
sess.run(globalinit)
...
def fillHists(tag, witherrors=options.computeHistErrors):
  ...
  if tag == 'prefit':
      if witherrors:
          results = sess.run([normfullcentral, nexpfullcentral, nexpsigcentral, nexpbkgcentral, nexpfullerrpre, nexpsigerrpre, nexpbkgerrpre, normfullerrpre])
      else:
          results = sess.run([normfullcentral, nexpfullcentral, nexpsigcentral, nexpbkgcentral]) + [None, None, None, None]
  else:
      if witherrors:
          results = sess.run([normfull, nexpfull, nexpsig, nexpbkg, nexpfullerr, nexpsigerr, nexpbkgerr, normfullerr])
      else:
          results = sess.run([normfull, nexpfull, nexpsig, nexpbkg]) + [None, None, None, None]

  # Print results from sess.run()
  # print("Results from sess.run():", results)

  # Unpack results
  try:
      (normfullval, nexpfullval, nexpsigval, nexpbkgval, 
        nexpfullerrval, nexpsigerrval, nexpbkgerrval, normfullerrval) = results
  except ValueError as e:
      print(f"Error unpacking results: {e}")
      # print("Results:", results)
      raise

  # Check for None values and raise ValueError if any
  missing_values = []
  if nexpfullval is None:
      missing_values.append('nexpfullval')
  if nexpsigval is None:
      missing_values.append('nexpsigval')
  if nexpbkgval is None:
      missing_values.append('nexpbkgval')
  if nexpfullerrval is None and witherrors:
      missing_values.append('nexpfullerrval')
  if nexpsigerrval is None and witherrors:
      missing_values.append('nexpsigerrval')
  if nexpbkgerrval is None and witherrors:
      missing_values.append('nexpbkgerrval')

  if missing_values:
      raise ValueError(f"One or more of the expected arrays is None: {', '.join(missing_values)}")

  # Print the arrays before using them
  print("nexpfullval:", nexpfullval)
  print("nexpsigval:", nexpsigval)
  print("nexpbkgval:", nexpbkgval)
  print("nexpfullerrval:", nexpfullerrval)
  print("nexpsigerrval:", nexpsigerrval)
  print("nexpbkgerrval:", nexpbkgerrval)

    # # hists for fit channels
    # expfullHist = ROOT.TH1D('expfull_%s' % tag, '', nbins, -0.5, float(nbins) - 0.5)
    # hists.append(expfullHist)
    # array2hist(nexpfullval[:nbins], expfullHist, errors=nexpfullerrval[:nbins] if nexpfullerrval is not None else None)
    
    # expsigHist = ROOT.TH1D('expsig_%s' % tag, '', nbins, -0.5, float(nbins) - 0.5)
    # hists.append(expsigHist)
    # array2hist(nexpsigval[:nbins], expsigHist, errors=nexpsigerrval[:nbins] if nexpsigerrval is not None else None)
    
    # expbkgHist = ROOT.TH1D('expbkg_%s' % tag, '', nbins, -0.5, float(nbins) - 0.5)
    # hists.append(expbkgHist)
    # array2hist(nexpbkgval[:nbins], expbkgHist, errors=nexpbkgerrval[:nbins] if nexpbkgerrval is not None else None)
  ...
  for iproc,proc in enumerate(procs):
    ...
    # Check if normprocerrval is None and handle accordingly
    if normprocerrval is not None:
        array2hist(normprocval[:nbins], expHist, errors=normprocerrval[:nbins])
    else:
        array2hist(normprocval[:nbins], expHist)    
    ...
    if nbinsmasked > 0 and normprocval[nbins:].sum() > 0:
      ...
      if normprocerrval is not None:
          ...
          array2hist(normprocval[nbins:], expHist_masked)
...
for itoy in range(ntoys):
  ...
  for ifit in range(2):
    ...
    print("\nstatus = %i, errstatus = %i, nllval = %f, nllvalfull = %f, edmval = %e, mineigval = %e, mineigvalinv = %e" % (status,errstatus,nllval,nllvalfull,edmval,mineigval,mineigvalinv))  
  ...
  for output, outputname, outvals,outvalsprefit,invhessoutval in zip(outputs, outputnames, outvalss,outvalssprefit,invhessoutvals):
    ...
    if errstatus==0:
      ...
      print(f"\tNumber of Outputs: {nout}")
      ...
      if not options.toys > 1:
        ...
        # fill_hist_from_array(correlationMatrix, correlationHist)
  ...
  if options.doImpacts and not options.toys > 1:
    ...
    for output, outputname, nuisanceimpactoutval, nuisancegroupimpactoutval in zip(outputs,outputnames,nuisanceimpactoutvals,nuisancegroupimpactoutvals):
      ...
      # Set Y-axis labels, decoding bytes to string if necessary
      for isyst, syst in enumerate(systs):
        if isinstance(syst, bytes):
  ...
  print("\nField Name = Value ± Uncertainty (+ Minos Up - Minos Down)")
  for output,outputname, outvals,outsigmas,minosups,minosdowns,outgenvals,toutvals,touterrs,toutminosups,toutminosdowns,toutgenvals in zip(
    ...
    for name,outval,outma,minosup,minosdown,outgenval,toutval,touterr,toutminosup,toutminosdown,toutgenval in zip(outputname,outvals,outsigmas,minosups,minosdowns,outgenvals,toutvals,touterrs,toutminosups,toutminosdowns,toutgenvals):
      ...
      if itoy==0:
        ...
        print(f"\n{name} = {outval: .6e} ± {outma: .6f} (+{minosup: .6f} -{minosdown: .6f})")
  ...
  if(nsyst > 0):
    ...
    syst_width = 20  # Adjust based on the longest `syst` string
    value_width = 10
    uncertainty_width = 10
    print("\n\nSystematic ".ljust(syst_width) + " = Value".ljust(value_width + 3) + " ± Uncertainty ".ljust(uncertainty_width + 3) + "(+ Minos Up - Minos Down) (Reference In = Ref. Value)\n")
  ...
  for syst,thetaval,theta0val,sigma,minosup,minosdown,thetagenval, tthetaval,ttheta0val,tthetaerr,tthetaminosup,tthetaminosdown,tthetagenval in zip(systs,thetavals,theta0vals,thetasigmasv,thetaminosups,thetaminosdowns,thetavalsgen, tthetavals,ttheta0vals,tthetaerrs,tthetaminosups,tthetaminosdowns,tthetagenvals):
    ...
    if itoy==0:
      ...
      if isinstance(syst, bytes):
        syst = syst.decode('utf-8')
      print(f"{syst.ljust(syst_width)} = {thetaval: {value_width}.6f} ± {sigma: {uncertainty_width}.6f} (+{minosup: .6f} -{minosdown: .6f}) ({syst}_In = {theta0val: .6f})\n")
  ...
  for var in options.scan:
    print("\nrunning profile likelihood scan for %s" % var)
