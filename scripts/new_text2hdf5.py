...
print(f"\nnbins = {nbins}, nbinsfull = {nbinsfull}, nproc = {nproc}, nsyst = {nsyst}\n")
...
for chan in chans:
  ...
  if not chan in options.maskedChan:
    ...
    # # Load histogram using uproot and convert to numpy arrays
    # data_obs_chan_hist = MB.getShape(chan, options.dataname)
    # data_obs_chan = uproot.open(data_obs_chan_hist).to_numpy(flow=False)[0].astype(dtype)

    # Directly convert the TH1F to NumPy without uproot
    data_obs_chan_hist = MB.getShape(chan, options.dataname)
    data_obs_chan = np.array(data_obs_chan_hist)[1:-1].astype(dtype).ravel()  # Excluding underflow/overflow bins
    
    nbinschan = data_obs_chan.shape[0]

    # Adding print statements for debugging
    # print(f"Channel: {chan}")
    # print(f"Histogram Name: {options.dataname}")
    # print(f"Data obs Array Shape: {data_obs_chan.shape}")
    # print(f"First 5 Entries of Data obs: {data_obs_chan[:5]}")
    # print(f"Number of bins: {nbinschan}\n")
    ...
    if options.externalCovariance:
        ...
        # data_cov_chan_hist = MB.getShape(chan, options.covname)
        # data_cov_chan = uproot.open(data_cov_chan_hist).to_numpy(flow=False).astype(dtype)

        # Accessing the histogram
        data_cov_chan_hist = MB.getShape(chan, options.covname)

        # Convert the TH1F histogram to a NumPy array
        data_cov_chan = np.array(data_cov_chan_hist)[1:-1].astype(dtype)

        # Adding print statements for debugging
        # print(f"Channel: {chan}")
        # print(f"Histogram Name: {options.covname}")
        # print(f"Data Covariance Array Shape: {data_cov_chan.shape}")
        # print(f"First 5 Entries of Data Covariance: {data_cov_chan[:5]}\n")
  ...
  for iproc,proc in enumerate(procs):
    ...
    # # Get histogram using uproot and convert to numpy array, excluding overflow/underflow bins
    # norm_chan_hist = MB.getShape(chan, proc)
    # # norm_chan = hist2array(norm_chan_hist, include_overflow=False).ravel().astype(dtype)   
    # norm_chan = uproot.open(norm_chan_hist).to_numpy(flow=False)[0].astype(dtype).ravel()
    ...
    # Convert the TH1F histogram to a NumPy array
    norm_chan = np.array(norm_chan_hist)[1:-1].astype(dtype).ravel()

    # Adding print statements for debugging
    print(f"Channel: {chan}, Process: {proc}")
    print(f"Norm Channel Histogram Name: {norm_chan_hist.GetName()}\n")
    # print(f"Norm Channel Array Shape: {norm_chan.shape}")
    # print(f"First 5 Entries of Norm Channel Array: {norm_chan[:5]}\n")

    # print(f"options.maskedChan: {options.maskedChan}\n")
    # if not chan in options.maskedChan:
    if chan not in options.maskedChan: 
      ...
      #   sumw2_chan = uproot.open(sumw2_chan_hist).to_numpy(flow=False)[1].ravel().astype(dtype)
      ...
      # Check if the histogram has sum of squares of weights (Sumw2)
      if norm_chan_hist.GetSumw2().GetSize() > 0:
          # print("Proper uncertainties present.")

          # Clone the histogram to manipulate it safely
          sumw2_chan_hist = norm_chan_hist.Clone()

          if sumw2_chan_hist.InheritsFrom('TH1F'):
              # Handling the fact that GetSumw2 returns a TArrayD but TH1F expects a Float_t *
              sumw2f = [sumw2_chan_hist.GetSumw2()[i] for i in range(sumw2_chan_hist.GetSumw2().GetSize())]
              sumw2f = array('f', sumw2f)
              sumw2_chan_hist.Set(sumw2_chan_hist.GetSumw2().GetSize(), sumw2f)
          else:
          ...
          sumw2_chan = np.array(sumw2_chan_hist)[1:-1].ravel().astype(dtype)

          # Print statements for debugging
          # print(f"\nSumw2 Channel Histogram Name: {sumw2_chan_hist.GetName()}")
          # print(f"Sumw2 Channel Array Shape: {sumw2_chan.shape}")
          # print(f"First 5 Entries of Sumw2 Channel Array: {sumw2_chan[:5]}\n")

    #   else:
    #       print("Warning: Sumw2 not filled for histograms, using fallback method for computing uncertainties.")
    #       print(proc, "is problematic")
    #       print(norm_chan_hist.GetSumw2().GetSize())

    #       # Compute uncertainties manually
    ...
    # norm_chan_hist.Delete()
          ...
          print("\nWarning: Sumw2 not filled for histograms, using fallback method for computing uncertainties.")
          print(f"Process: {proc}, is problematic.")
          print(f"Sumw2 Size: {norm_chan_hist.GetSumw2().GetSize()}\n")
          ...
          # Print statements for debugging
          print(f"\nnentries_chan Shape: {nentries_chan.shape}")
          print(f"First 5 Entries of nentries_chan: {nentries_chan[:5]}")
          print(f"First 5 Entries of sumw2_chan: {sumw2_chan[:5]}\n")

          # Clear the variable as it's no longer needed
          nentries_chan = None
    ...
    for isyst,syst in enumerate(systdata):
      ...
      if stype in ['lnN', 'lnNNoProfile', 'lnNNoConstraint']:
        ...
        systup_chan = np.array(systup_chan_hist)[1:-1].ravel().astype(dtype) 
        # Debugging prints
        # print(f"\nSystUp Channel Histogram Name: {name}Up")
        # print(f"SystUp Channel Array Shape: {systup_chan.shape}")
        # print(f"First 5 Entries of SystUp Channel Array: {systup_chan[:5]}\n")       
        ...
        systdown_chan = np.array(systdown_chan_hist)[1:-1].ravel().astype(dtype)
        systdown_chan_hist.Delete()
        # Debugging prints
        # print(f"\nSystDown Channel Histogram Name: {name}Down")
        # print(f"SystDown Channel Array Shape: {systdown_chan.shape}")
        # print(f"First 5 Entries of SystDown Channel Array: {systdown_chan[:5]}\n")
        ...
        # Clipping systematic variations
